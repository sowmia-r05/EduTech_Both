...
"""
    ...
    if weak_topics:
        prompt += f"\nWEAK TOPICS (<65%): {', '.join(weak_topics)}"
...
{{
  "summary": "2-3 sentence overall summary of their cumulative performance in {subject}",
  "strengths": ["strength 1 with specific topic/score data", "strength 2", "strength 3"],
  "areas_for_improvement": [
    {{"issue": "specific weakness", "how_to_improve": "concrete actionable tip"}},
    {{"issue": "specific weakness 2", "how_to_improve": "concrete actionable tip 2"}}
  ],
  "study_tips": ["specific tip 1 referencing {subject}", "specific tip 2", "specific tip 3"],
  "encouragement": "1-2 warm, motivating sentences personalised to their journey",
  "trend": "{trend}",
  "topic_highlights": ["1 sentence about their best topic", "1 sentence about a topic to focus on"]
}}

Rules:
- Be specific — reference actual percentages, topic names, quiz counts
- Keep tone warm, encouraging, age-appropriate for Year {year_level}
...
- If subject is "Overall", cover patterns across all subjects
...
def main():
    ...
    if not tests:
        ...
        print(json.dumps({
            ...
            "feedback": {
                ...
                "strengths": [],
                "areas_for_improvement": [],
                "study_tips": ["Start with a subject you feel confident in.", "Try at least one quiz per subject.", "Review your answers after each quiz."],
    ...
    try:
        ...
        print(json.dumps({"success": False, "error": f"Gemini init failed: {e}"}))
        sys.exit(0)
    ...
    try:
        ...
        print(json.dumps({"success": False, "error": f"Gemini API error: {e}"}))
        sys.exit(0)
    ...
    try:
        ...
        if match:
            try:
                ...
                print(json.dumps({"success": False, "error": "Could not parse Gemini JSON output", "raw": raw_output[:500]}))
                sys.exit(0)
        else:
            print(json.dumps({"success": False, "error": "Gemini returned non-JSON output", "raw": raw_output[:500]}))
            sys.exit(0)
