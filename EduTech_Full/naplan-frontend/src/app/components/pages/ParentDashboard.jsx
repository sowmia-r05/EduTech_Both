 ...
 *
 * FIXES APPLIED:
 *  ✅ AddChildModal: emailNotifications init changed from `child.email_notifications ?? false` → `false`
 *  ✅ EditChildModal: added emailNotifications state, wired into handleSubmit, added UI toggle
 *  ✅ EditChildModal: removed broken JSX attribute (pseudo-code pasted into div tag)
 *  ✅ Both modals: email + consent checkboxes properly aligned in a styled box
 */
...
const AVATAR_COLORS = [
  "#7C3AED","#0EA5E9","#10B981","#F59E0B","#EF4444","#EC4899","#14B8A6",
];
...
function lastActiveBadge(days) {
  if (days === null || days === undefined) return { bg: "#F9FAFB", color: "#9CA3AF" };
...
const Card = ({ children, borderColor }) => (
  <div style={{
    ...
    border: `1px solid ${borderColor}55`, borderTop: `3px solid ${borderColor}`,
    padding: "20px 22px", flex: "1 1 280px", minWidth: "280px",
...
const CardTop = ({ icon, iconBg, label, subLabel, accent, bigNum, bigNumSub, bigNumPrefix }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px" }}>
    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
      <div style={{ width: "34px", height: "34px", borderRadius: "9px", background: iconBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {icon}
      </div>
      <div>
        <div style={{ fontSize: "11px", fontWeight: 700, color: accent, letterSpacing: "0.07em", textTransform: "uppercase" }}>{label}</div>
    ...
    <div style={{ textAlign: "right" }}>
      ...
      <span style={{ fontSize: "38px", fontWeight: 900, color: "#111827", lineHeight: 1 }}>{bigNum}</span>
      {bigNumSub && <span style={{ fontSize: "15px", color: "#9CA3AF", marginLeft: "1px" }}>{bigNumSub}</span>}
    </div>
  </div>
);

const EmptyRow = ({ message }) => (
  <div style={{ padding: "24px 0", textAlign: "center", color: "#D1D5DB", fontSize: "13px" }}>{message}</div>
);
...
function ChildrenCard({ childList }) {
  ...
  return (
    <Card borderColor="#7C3AED">
      <CardTop accent="#7C3AED" iconBg="#EDE9FE" label="Children" subLabel="Profiles registered" bigNum={childList.length}
      ...
      />
      {childList.length === 0 && <EmptyRow message="No children added yet" />}
      {active > 0 && (
        <div style={{ marginBottom: "12px" }}>
          <span style={{ fontSize: "12px", fontWeight: 700, padding: "3px 12px", borderRadius: "20px", background: "#ECFDF5", color: "#059669", border: "1px solid #A7F3D0", display: "inline-block", marginBottom: "8px" }}>● {active} Active</span>
          <div style={{ display: "flex", flexDirection: "column", gap: "5px", paddingLeft: "4px" }}>
            {childList.filter((c) => c.status === "active").map((c) => (
              <div key={c.id} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <div style={{ width: "24px", height: "24px", borderRadius: "50%", background: AVATAR_COLORS[childList.indexOf(c) % AVATAR_COLORS.length], display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "10px", fontWeight: 700, flexShrink: 0 }}>{ini(c.name)}</div>
                <span style={{ fontSize: "13px", color: "#374151", fontWeight: 500 }}>{cap(c.name)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {trial > 0 && (
        <div>
          <span style={{ fontSize: "12px", fontWeight: 700, padding: "3px 12px", borderRadius: "20px", background: "#FFF7ED", color: "#D97706", border: "1px solid #FDE68A", display: "inline-block", marginBottom: "8px" }}>● {trial} Trial</span>
          <div style={{ display: "flex", flexDirection: "column", gap: "5px", paddingLeft: "4px" }}>
...
function QuizzesCard({ childList }) {
  ...
  return (
    <Card borderColor="#8B5CF6">
      <CardTop accent="#8B5CF6" iconBg="#EDE9FE" label="Quizzes" subLabel="Total completed" bigNum={total}
        icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#8B5CF6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>}
      />
        ...
        <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "14px", paddingRight: "2px" }}>
          {childList.map((child) => (
            <div key={child.id} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ fontSize: "15px", fontWeight: 600, color: "#374151", width: "72px", flexShrink: 0 }}>{child.name}</span>
              <div style={{ flex: 1, height: "8px", background: "#F3F4F6", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ height: "100%", borderRadius: "4px", width: (child.quizzes || 0) > 0 ? `${((child.quizzes || 0) / maxQ) * 100}%` : "0%", background: "linear-gradient(90deg,#7C3AED,#A78BFA)", transition: "width 0.6s ease" }} />
              </div>
              <span style={{ fontSize: "15px", fontWeight: 700, color: (child.quizzes || 0) > 0 ? "#111827" : "#D1D5DB", width: "20px", textAlign: "right", flexShrink: 0 }}>{child.quizzes || 0}</span>
            </div>
          ))}
        </div>
      )}
...
function ScoresCard({ childList }) {
  ...
  return (
    <Card borderColor="#0EA5E9">
      <CardTop accent="#0EA5E9" iconBg="#E0F2FE" label="Scores" subLabel="Per child breakdown"
        bigNum={scored.length ? `${avg}%` : "—"} bigNumPrefix={scored.length ? "AVG" : undefined}
        icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>}
      />
      {childList.length === 0 ? <EmptyRow message="No score data" /> : (
        <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "13px", paddingRight: "2px" }}>
          {childList.map((child) => (
            <div key={child.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", flex: 1 }}>
                <span style={{ fontSize: "14px", color: "#374151", flexShrink: 0, minWidth: "58px" }}>{child.name}</span>
                {child.score !== null && child.score !== undefined ? (
                  <>
                    <div style={{ flex: 1, height: "7px", background: "#F3F4F6", borderRadius: "4px", overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: "4px", width: `${child.score}%`, background: "linear-gradient(90deg,#F59E0B,#FBBF24)" }} />
                    </div>
                    <span style={{ fontSize: "13px", fontWeight: 700, color: "#F59E0B", minWidth: "34px", textAlign: "right", flexShrink: 0 }}>{child.score}%</span>
                  </>
                ) : (
                  <>
                    <span style={{ fontSize: "13px", color: "#9CA3AF" }}>No attempts</span>
                    <span style={{ marginLeft: "auto", fontSize: "14px", color: "#D1D5DB" }}>—</span>
                  </>
                )}
              </div>
            </div>
      ...
      {leader && (
        <div style={{ marginTop: "14px", background: "#FFFBEB", borderRadius: "9px", padding: "8px 12px", display: "flex", alignItems: "center", gap: "6px", border: "1px solid #FDE68A", flexShrink: 0 }}>
          <span>⭐</span>
          <span style={{ fontSize: "13px", color: "#92400E", fontWeight: 600 }}>{leader.name} leading at {leader.score}%</span>
        </div>
      )}
    </Card>
  );
...
function LastActiveCard({ childList }) {
  ...
  const hasStale = childList.some((c) => c.lastActiveDays === null || c.lastActiveDays > 3);
  return (
    <Card borderColor="#10B981">
      <CardTop accent="#10B981" iconBg="#D1FAE5" label="Last Active" subLabel={`${todayCount} active today`}
      ...
      />
      {childList.length === 0 ? <EmptyRow message="No activity data" /> : (
        <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: "13px", paddingRight: "2px" }}>
          {childList.map((child) => {
            ...
            return (
              <div key={child.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: "12px", fontWeight: 600, padding: "3px 10px", borderRadius: "10px", background: badge.bg, color: badge.color, flexShrink: 0 }}>{label}</span>
              </div>
            );
          })}
        </div>
      ...
      {hasStale && childList.length > 0 && (
        <div style={{ marginTop: "14px", background: "#FFF1F1", borderRadius: "9px", padding: "8px 12px", display: "flex", alignItems: "center", gap: "6px", border: "1px solid #FECACA", flexShrink: 0 }}>
          <span>⚡</span>
          <span style={{ fontSize: "12px", color: "#B91C1C", fontWeight: 600 }}>Some children haven't practiced recently</span>
        </div>
      )}
    </Card>
  );
  ...
  return (
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      <button onClick={() => setOpen((v) => !v)}
        style={{ background: open ? "#EDE9FE" : "transparent", border: "none", borderRadius: "6px", cursor: "pointer", padding: "4px 6px", display: "flex", alignItems: "center", color: "#9CA3AF", transition: "background 0.15s" }}
...
function ChildCard({ child, colorIndex, onEdit, onDelete, onViewResults, onFreeSample, onBuyBundle }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #E5E7EB", boxShadow: "0 2px 10px rgba(0,0,0,0.06)", padding: "20px 22px", display: "flex", flexDirection: "column", gap: "0", flex: "1 1 280px", minWidth: "280px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}>
        <span style={{ fontSize: "12px", fontWeight: 600, padding: "3px 10px", borderRadius: "20px", background: isActive ? "#ECFDF5" : "#FFF7ED", color: isActive ? "#059669" : "#D97706", border: `1px solid ${isActive ? "#A7F3D0" : "#FDE68A"}`, flexShrink: 0 }}>{isActive ? "Active" : "Trial"}</span>
        {isActive ? <span style={{ fontSize: "13px", color: "#6B7280" }}>Bundle purchased ✓</span> : <span style={{ fontSize: "13px", color: "#7C3AED", fontWeight: 600, cursor: "pointer" }} onClick={() => onBuyBundle?.(child)}>Upgrade to Full Access →</span>}
      </div>
      <div style={{ marginBottom: "14px" }}>
        ...
        <div style={{ height: "7px", background: "#F3F4F6", borderRadius: "4px", overflow: "hidden" }}>
          <div style={{ height: "100%", borderRadius: "4px", width: `${child.score || 0}%`, background: isActive ? "#EF4444" : "#D1D5DB", transition: "width 0.6s ease" }} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "auto" }}>
        {isActive ? (
          <>
            <button onClick={() => onViewResults?.(child)}
              style={{ flex: 1, padding: "10px 0", borderRadius: "9px", background: "linear-gradient(135deg, #6366F1, #4F46E5)", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: "7px", boxShadow: "0 2px 8px rgba(99,102,241,0.35)", transition: "opacity 0.15s, transform 0.1s" }}
              onMouseEnter={(e) => { e.currentTarget.style.opacity = "0.88"; e.currentTarget.style.transform = "translateY(-1px)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; e.currentTarget.style.transform = "translateY(0)"; }}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v18h18"/><polyline points="18 9 12 15 9 12 3 18"/></svg>
              View Results
            </button>
            <button onClick={() => onBuyBundle?.(child)} style={{ flex: 1, padding: "10px 0", borderRadius: "9px", background: "#fff", border: "1.5px solid #D1D5DB", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#374151", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>🛒 Buy Bundle</button>
          </>
        ) : (
          <>
            <button onClick={() => onFreeSample?.(child)} style={{ flex: 1, padding: "10px 0", borderRadius: "9px", background: "#fff", border: "1.5px solid #D1D5DB", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#374151", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>Free Sample Test</button>
            <button onClick={() => onBuyBundle?.(child)} style={{ flex: 1, padding: "10px 0", borderRadius: "9px", background: "#059669", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>🛒 Buy Bundle</button>
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "0 0 16px" }}>
        ...
        <button onClick={onAddChild} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 14px", borderRadius: "9px", background: "#7C3AED", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#fff", boxShadow: "0 1px 4px rgba(124,58,237,0.3)" }}>
      ...
      {childList.length === 0 ? (
        <div style={{ background: "#fff", borderRadius: "14px", border: "1.5px dashed #E5E7EB", padding: "48px", textAlign: "center" }}>
          ...
          <button onClick={onAddChild} style={{ padding: "10px 24px", borderRadius: "9px", background: "#7C3AED", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#fff" }}>Add Child</button>
        </div>
      ) : (
    ...
    <ModalOverlay onClose={onCancel} maxWidth="400px">
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <button onClick={onCancel} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: "#F3F4F6", border: "none", cursor: "pointer", fontSize: "14px", fontWeight: 600, color: "#374151" }}>Cancel</button>
          <button onClick={onConfirm} disabled={loading} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: loading ? "#FDE68A" : "#D97706", border: "none", cursor: loading ? "not-allowed" : "pointer", fontSize: "14px", fontWeight: 600, color: "#fff" }}>{loading ? "Redirecting…" : "Retry Payment"}</button>
        </div>
      </div>
    </ModalOverlay>
  );
      ...
      <div style={{ marginTop: "40px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
          <div>
            ...
            {!collapsed && <p style={{ fontSize: "12px", color: "#9CA3AF", margin: "3px 0 0" }}>{filtered.length} transaction{filtered.length !== 1 ? "s" : ""}{" · Click "}<span style={{ color: "#D97706", fontWeight: 600 }}>Pending</span>{" / "}<span style={{ color: "#EF4444", fontWeight: 600 }}>Failed</span>{" rows to retry"}</p>}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            {!collapsed && payments.length > 0 && (
              <div ref={filterRef} style={{ position: "relative" }}>
                <button onClick={() => setFilterOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: "7px", padding: "7px 13px", borderRadius: "9px", cursor: "pointer", background: hasFilter ? "#EDE9FE" : "#fff", border: `1px solid ${hasFilter ? "#C4B5FD" : "#E5E7EB"}`, fontSize: "13px", fontWeight: 600, color: hasFilter ? "#6D28D9" : "#374151", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                  ...
                  Filter
                  {hasFilter && <span style={{ background: "#7C3AED", color: "#fff", borderRadius: "10px", padding: "0 6px", fontSize: "11px", fontWeight: 700 }}>1</span>}
                </button>
                {filterOpen && (
                  <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 300, background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", boxShadow: "0 12px 32px rgba(0,0,0,0.12)", padding: "16px 18px", minWidth: "200px" }}>
                    <div style={{ fontSize: "11px", fontWeight: 700, color: "#9CA3AF", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: "10px" }}>Filter by Status</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: hasFilter ? "12px" : 0 }}>
                      {statuses.map((s) => (
                        <button key={s} onClick={() => { setActiveStatus(s); setFilterOpen(false); }} style={{ padding: "4px 12px", borderRadius: "20px", cursor: "pointer", fontSize: "12px", fontWeight: 600, background: activeStatus === s ? "#7C3AED" : "#F3F4F6", color: activeStatus === s ? "#fff" : "#374151", border: "none" }}>{s}</button>
                      ))}
                    </div>
                    {hasFilter && <button onClick={() => { setActiveStatus("All"); setFilterOpen(false); }} style={{ width: "100%", padding: "7px", borderRadius: "8px", background: "#FFF1F2", border: "1px solid #FECDD3", color: "#BE123C", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}>Clear filter</button>}
                  </div>
                )}
              </div>
            )}
            <button onClick={() => setCollapsed((v) => !v)} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 13px", borderRadius: "9px", cursor: "pointer", background: collapsed ? "#EDE9FE" : "#fff", border: `1px solid ${collapsed ? "#C4B5FD" : "#E5E7EB"}`, fontSize: "12px", fontWeight: 600, color: collapsed ? "#6D28D9" : "#6B7280", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transition: "transform 0.2s", transform: collapsed ? "rotate(180deg)" : "rotate(0deg)" }}><polyline points="18 15 12 9 6 15"/></svg>
        ...
        {!collapsed && (
          <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #E5E7EB", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflow: "hidden" }}>
            ...
            {filtered.length === 0 ? (
              <div style={{ padding: "32px", textAlign: "center", color: "#9CA3AF", fontSize: "14px" }}>{payments.length === 0 ? "No payment history yet." : "No transactions match your filters."}</div>
            ) : (
              filtered.map((p, i) => {
                ...
                return (
                  <div key={p.id} onClick={() => isRetryable && setRetryTarget(p)}
                    style={{ display: "grid", gridTemplateColumns: "1.6fr 2.2fr 1fr 1fr", padding: "13px 20px", borderBottom: rowBorder, alignItems: "center", background: rowBaseBg, transition: "background 0.12s", cursor: isRetryable ? "pointer" : "default" }}
                    onMouseEnter={(e) => { if (isRetryable) e.currentTarget.style.background = "#FFFBEB"; else if (isDeletedChild) e.currentTarget.style.background = "#FEE2E2"; else e.currentTarget.style.background = "#FAFAFA"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = rowBaseBg; }}
                  >
                    <div>
                      <span style={{ fontSize: "13px", color: isDeletedChild ? "#B91C1C" : "#374151", fontWeight: 500 }}>{p.date.split(",")[0]}</span>
                      <span style={{ fontSize: "11px", color: isDeletedChild ? "#EF9999" : "#9CA3AF", display: "block", marginTop: "1px" }}>{p.date.includes(" at ") ? p.date.split(" at ")[1] : p.date.split(", ").slice(1).join(", ")}</span>
                    </div>
                    <div>
                      <span style={{ fontSize: "13px", color: isDeletedChild ? "#B91C1C" : "#374151" }}>{p.description}</span>
                      {isDeletedChild ? <span style={{ fontSize: "11px", color: "#EF4444", display: "block", marginTop: "2px", fontWeight: 600 }}>⚠ Child account deleted</span> : p.child && p.child !== "—" ? <span style={{ fontSize: "11px", color: "#9CA3AF", display: "block", marginTop: "1px" }}>{p.child}</span> : null}
                    </div>
                    <span style={{ fontSize: "14px", fontWeight: 700, color: isDeletedChild ? "#B91C1C" : "#111827" }}>{p.amount}</span>
                    <div><span style={{ fontSize: "11px", fontWeight: 700, padding: "4px 11px", borderRadius: "20px", background: st.bg, color: st.color, border: `1px solid ${st.border}`, display: "inline-block" }}>{p.status}</span></div>
                  </div>
                );
              })
            )}
        ...
        {collapsed && (
          <div style={{ background: "#fff", borderRadius: "10px", border: "1px solid #E5E7EB", padding: "12px 20px", display: "flex", alignItems: "center", gap: "14px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.7)", backdropFilter: "blur(2px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
        <div style={{ width: "36px", height: "36px", borderRadius: "50%", border: "3px solid #EDE9FE", borderTopColor: "#7C3AED", animation: "spin 0.7s linear infinite" }} />
        <span style={{ fontSize: "13px", color: "#7C3AED", fontWeight: 600 }}>Loading…</span>
      </div>
...
const INPUT_STYLE = {
  ...
  borderRadius: "9px", padding: "9px 12px", fontSize: "14px",
  ...
  return (
    <label style={{ display: "flex", alignItems: "flex-start", gap: "10px", cursor: "pointer" }}>
      <input
        ...
        checked={checked}
        onChange={onChange}
        style={{ marginTop: "2px", flexShrink: 0, width: "15px", height: "15px", accentColor: "#7C3AED", cursor: "pointer" }}
      />
...
function AddChildModal({ onClose, onAdd, loading }) {
  ...
  const [emailNotifications, setEmailNotifications] = useState(false); // ✅ FIXED: was `child.email_notifications ?? false`
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[a-z0-9_]{3,20}$/.test(cleanUser)) return setError("Username: 3–20 chars, letters/numbers/underscore only");
    ...
    <ModalOverlay onClose={onClose}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {/* ── Email + Consent block ── */}
          <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: "12px", padding: "14px 16px", display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            {/* Consent */}
            <div style={{ background: "#FFF7ED", border: "1px solid #FDE68A", borderRadius: "9px", padding: "12px 14px" }}>
              <CheckboxRow checked={consent} onChange={(e) => setConsent(e.target.checked)}>
                I have read and agree to the{" "}
                <span
                  onClick={() => setShowConsentPolicy(true)}
                  style={{ color: "#7C3AED", textDecoration: "underline", cursor: "pointer", fontWeight: 600 }}
                >
          ...
          {/* Error */}
          {error && (
            <div style={{ background: "#FFF1F2", border: "1px solid #FECDD3", borderRadius: "9px", padding: "10px 14px", fontSize: "13px", color: "#BE123C", fontWeight: 600 }}>{error}</div>
          )}

          {/* Actions */}
          <div style={{ display: "flex", gap: "10px", marginTop: "4px" }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: "#F3F4F6", border: "none", cursor: "pointer", fontSize: "14px", fontWeight: 600, color: "#374151" }}>Cancel</button>
            <button type="submit" disabled={loading} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: loading ? "#A78BFA" : "#7C3AED", border: "none", cursor: loading ? "not-allowed" : "pointer", fontSize: "14px", fontWeight: 600, color: "#fff" }}>{loading ? "Adding…" : "Add Child"}</button>
          </div>
        </div>
      </form>
    </ModalOverlay>
...
function EditChildModal({ child, onClose, onSave, loading }) {
  ...
  const [emailNotifications, setEmailNotifications] = useState(child.email_notifications ?? false); // ✅ NEW
  const [error, setError]                           = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ NEW: include email_notifications if changed
    ...
    <ModalOverlay onClose={onClose}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "10px", marginTop: "4px" }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: "#F3F4F6", border: "none", cursor: "pointer", fontSize: "14px", fontWeight: 600, color: "#374151" }}>Cancel</button>
            <button type="submit" disabled={loading} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: loading ? "#A78BFA" : "#7C3AED", border: "none", cursor: loading ? "not-allowed" : "pointer", fontSize: "14px", fontWeight: 600, color: "#fff" }}>{loading ? "Saving…" : "Save Changes"}</button>
          </div>
        </div>
      </form>
    </ModalOverlay>
  ...
  return (
    <ModalOverlay onClose={onCancel} maxWidth="400px">
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: "13px", color: "#6B7280", margin: "0 0 24px", lineHeight: 1.6 }}>This will permanently delete <strong>{child.name || "this child"}</strong>'s profile and all their quiz history. This action cannot be undone.</p>
        <div style={{ display: "flex", gap: "10px" }}>
          <button onClick={onCancel} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: "#F3F4F6", border: "none", cursor: "pointer", fontSize: "14px", fontWeight: 600, color: "#374151" }}>Cancel</button>
          <button onClick={onConfirm} disabled={loading} style={{ flex: 1, padding: "11px", borderRadius: "9px", background: loading ? "#FCA5A5" : "#EF4444", border: "none", cursor: loading ? "not-allowed" : "pointer", fontSize: "14px", fontWeight: 600, color: "#fff" }}>{loading ? "Deleting…" : "Delete"}</button>
        </div>
      </div>
    </ModalOverlay>
  );
      ...
      {bundles.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {bundles.map((bundle) => {
            ...
            return (
              <div key={bundle.bundle_id} style={{ border: "1px solid #E5E7EB", borderRadius: "12px", padding: "16px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", background: alreadyOwned ? "#F9FAFB" : "#fff" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "15px", fontWeight: 800, color: alreadyOwned ? "#9CA3AF" : "#7C3AED" }}>{price}</div>
                </div>
                {alreadyOwned ? (
                  <span style={{ padding: "8px 16px", borderRadius: "9px", background: "#F3F4F6", color: "#9CA3AF", fontSize: "13px", fontWeight: 600, flexShrink: 0 }}>Already Owned ✓</span>
                ) : (
                  <button onClick={() => onSelect(bundle)} disabled={isLoading} style={{ padding: "9px 18px", borderRadius: "9px", background: isLoading ? "#A7F3D0" : "#059669", border: "none", cursor: isLoading ? "not-allowed" : "pointer", fontSize: "13px", fontWeight: 600, color: "#fff", flexShrink: 0 }}>{isLoading ? "Redirecting…" : "Select & Pay"}</button>
                )}
              </div>
            );
          })}
...
function UserMenu({ user, onLogout, onAddChild, onChildLogin }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <div onClick={() => setOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#F9FAFB", borderRadius: "24px", padding: "4px 12px 4px 4px", border: "1px solid #E5E7EB", cursor: "pointer" }}>
        <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: "linear-gradient(135deg,#7C3AED,#0EA5E9)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "11px", fontWeight: 700 }}>{user.initials || "??"}</div>
        <span style={{ fontSize: "13px", fontWeight: 600, color: "#374151" }}>{user.name ? user.name.split(" ")[0] : "Account"}</span>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
      </div>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #E5E7EB", borderRadius: "12px", boxShadow: "0 12px 32px rgba(0,0,0,0.12)", zIndex: 400, minWidth: "180px", overflow: "hidden" }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #F3F4F6" }}>
            <div style={{ fontSize: "13px", fontWeight: 700, color: "#111827" }}>{user.name || "—"}</div>
            <div style={{ fontSize: "12px", color: "#9CA3AF", marginTop: "2px" }}>Parent Account</div>
          </div>
          <button onClick={() => { setOpen(false); onAddChild?.(); }} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", fontSize: "13px", color: "#374151", textAlign: "left" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#F9FAFB")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>
            Add Child
          </button>
          <button onClick={() => { setOpen(false); onChildLogin?.(); }} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", fontSize: "13px", color: "#374151", textAlign: "left" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#F9FAFB")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0EA5E9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>
            Child Login
          </button>
          <div style={{ height: "1px", background: "#F3F4F6", margin: "4px 0" }} />
          <button onClick={() => { setOpen(false); onLogout?.(); }} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "11px 16px", background: "none", border: "none", cursor: "pointer", fontSize: "13px", color: "#EF4444", textAlign: "left" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#FFF1F1")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    ...
    if (payment === "success") {
      ...
      if (sessionId) setSuccessSessionId(sessionId);
      loadChildren();
      loadPayments();
    }
  ...
  const handleEditRequest = (mappedChild) => {
    const raw = rawChildren.find((c) => String(c._id) === String(mappedChild._id || mappedChild.id));
    if (raw) setEditTarget(raw);
  };

  const practicePacks = children.reduce((sum, c) => sum + (c.entitled_bundle_ids?.length || 0), 0);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F3F4F6", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <main style={{ padding: "36px 48px", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "32px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#6B7280", margin: "5px 0 0" }}>Welcome back, <strong style={{ color: "#7C3AED" }}>{user.name || "—"}</strong> — manage children and access bundles</p>
          </div>
          <button onClick={() => navigate("/bundles")} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#fff", border: "1px solid #E5E7EB", borderRadius: "10px", padding: "9px 16px", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#374151", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            <div style={{ width: "22px", height: "22px", borderRadius: "5px", background: "linear-gradient(135deg,#7C3AED,#6D28D9)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
            </div>
            Practice Packs
            {practicePacks > 0 && <span style={{ background: "#7C3AED", color: "#fff", borderRadius: "12px", padding: "1px 8px", fontSize: "11px", fontWeight: 700 }}>{practicePacks}</span>}
          </button>
        </div>
      ...
      {showOnboarding && (
        <FreeTrialOnboarding
          parentToken={parentToken}
          onComplete={() => { loadChildren(); setShowOnboarding(false); const next = new URLSearchParams(searchParams); next.delete("onboarding"); setSearchParams(next, { replace: true }); }}
          onSkip={() => { setShowOnboarding(false); const next = new URLSearchParams(searchParams); next.delete("onboarding"); setSearchParams(next, { replace: true }); }}
        />
      )}
    </div>
  );
